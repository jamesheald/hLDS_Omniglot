...
from utils import instantiate_a_myosuite_environment
...
def get_train_state(models, params, args, model_label):
    ...
    if args.reload_state:
        ...
        state = checkpoints.restore_checkpoint(ckpt_dir = 'runs/' + args.reload_folder_name + '/' + model_label, target = state)
...
def apply_model(apply_fn, params_vae, data, A, gamma, state_myo, key):

    return apply_fn({'params': {'encoder': params_vae['params']['encoder']}}, data, params_vae['decoder'], A, gamma, state_myo, key)
    # return apply_fn(params_vae, data, params_vae['params']['decoder'], A, gamma, state_myo, key) - try this once model working

batch_apply_model = vmap(apply_model, in_axes = (None, None, 0, None, None, None, 0))
    
def loss_fn(params_vae, state_vae, state_myo, data, kl_weight, key):
    ...
    # apply the model
    output = batch_apply_model(state_vae.apply_fn, params_vae, data, A, gamma, state_myo, subkeys)
    ...
    mu_0 = 0
    log_var_0 = params_vae['prior_z_log_var']
    mu_1 = output['z_mean']
...
def train_step_vae(state_vae, state_myo, training_data, kl_weight, key):
    
    (loss, (all_losses, output)), grads = loss_grad_vae(state_vae.params, state_vae, state_myo, training_data, kl_weight, key)

    state_vae = state_vae.apply_gradients(grads = grads)
    
    return state_vae, all_losses, output['muscle_inputs']

train_step_vae_jit = jit(train_step_vae)

def run_myosuite(env, muscle_inputs):
    ...
    fingertip_xyz = onp.empty((n_characters, n_timesteps, 3))
    ...
    for character in range(n_characters):
        ...
        for timestep in range(n_timesteps):
            ...
            obs, reward, done, info = env.step(onp.copy(muscle_inputs[character,timestep,:]))
        
            # store the fingertip position
            fingertip_xyz[character,timestep,:] = onp.copy(env.get_obs_dict(env.sim)['tip_pos'])

    return env, np.array(fingertip_xyz)

def loss_fn_myo(params, state, muscle_inputs, target_fingertip_xyz):

    def predict_fingertip_xyz(params, state, muscle_inputs):

        carry = params['params']['carry_init']
        inputs = muscle_inputs

        _, fingertip_xyz = lax.scan(lambda carry, muscle_inputs: state.apply_fn(params, carry, muscle_inputs), carry, muscle_inputs)
        # _, fingertip_xyz = lax.scan(state.apply_fn, carry, muscle_inputs)

        return fingertip_xyz

    batch_predict_fingertip_xyz = vmap(predict_fingertip_xyz, in_axes = (None, None, 0))

    # predict the fingertip position using the learned myosuite dynamics model
    predicted_fingertip_xyz = batch_predict_fingertip_xyz(params, state, muscle_inputs)

    # mean squared error between the predicted and actual (i.e. myosuite) fingertip position
    loss = ((target_fingertip_xyz - predicted_fingertip_xyz)**2).mean()

    return loss

loss_grad_myo = value_and_grad(loss_fn_myo)

def train_step_myo(state, muscle_inputs, fingertip_position):
    
    loss, grads = loss_grad_myo(state.params, state, muscle_inputs, fingertip_position)
...
def optimise_model(models, params, train_dataset, validate_dataset, args, key):
    ...
    env = instantiate_a_myosuite_environment(args, next(subkey))
    ...
    # write image from the untrained model to tensorboard to have a baseline as a reference
    key, validation_subkeys = keyGen(key, n_subkeys = 1)
    kl_weight = 0
    _, (validation_losses, output) = eval_step_jit(state_vae.params, state_vae, state_myo, validate_dataset, kl_weight, next(validation_subkeys))
    write_images_to_tensorboard(writer, output, args, validate_dataset, epoch = 0)        
    ...
    for epoch in range(args.n_epochs):
        ...
        training_losses = {'total': 0, 'cross_entropy': 0, 'kl': 0, 'kl_prescale': 0, 'mse': 0}
        ...
        for batch in range(1, args.n_batches + 1):
            ...
            env, fingertip_position = run_myosuite(env, muscle_inputs)

            # train the myosuite dynamics model
            mse = np.array([1,1])
            # state_myo, mse = train_step_myo_jit(state_myo, muscle_inputs, fingertip_position)

            # training losses (average of 'print_every' batches)
            training_losses = tree_map(lambda x, y: x + y / args.print_every, training_losses, all_losses | {'mse': mse})
        ...
        if epoch % 50 == 0:
            ...
            # save checkpoint
            checkpoints.save_checkpoint(ckpt_dir = 'runs/' + args.folder_name + '/' + 'vae', target = state_vae, step = epoch)
            checkpoints.save_checkpoint(ckpt_dir = 'runs/' + args.folder_name + '/' + 'myosuite', target = state_myo, step = epoch)
